...
import { getLatestRelease, generateDownloadInfo, generateAdditionalDownloads, type DownloadInfo } from '../utils/github'
...
function getIconForOS(os: string) {
  if (os.toLowerCase().includes('windows')) return icons.windows
  if (os.toLowerCase().includes('mac')) return icons.macos
  if (os.toLowerCase().includes('linux')) return icons.linux
...
export function Download() {
  ...
  const [additionalDownloads, setAdditionalDownloads] = useState<DownloadInfo[]>([])
  ...
  useEffect(() => {
    ...
    const fetchReleases = async () => {
      try {
        ...
        setDownloads(generateDownloadInfo(release))
        setAdditionalDownloads(generateAdditionalDownloads(release))
      } catch (error) {
        console.error('Erro ao carregar releases:', error)
        // Fallback já está incluído nas funções generate
        setDownloads(generateDownloadInfo(null))
        setAdditionalDownloads(generateAdditionalDownloads(null))
      } finally {
        setIsLoading(false)
      }
    }
      ...
      <div className="max-w-6xl mx-auto text-center relative">
        ...
        <div className="mb-12">
          <div className="text-center mb-12">
            <h3 className="text-2xl font-semibold text-plaxo-text mb-4">
              Ou baixe sem sincronização
            </h3>
            <p className="text-plaxo-text-secondary">
              Use o Plaxo OTP apenas localmente, sem backup na nuvem
            </p>
          </div>
            ...
            {isLoading ? (
              ...
              Array.from({ length: 3 }).map((_, index) => (
                <div key={index} className="card animate-pulse">
                  <div className="w-16 h-16 bg-plaxo-surface rounded-2xl mx-auto mb-6"></div>
                  <div className="h-6 bg-plaxo-surface rounded mb-2"></div>
                  <div className="h-4 bg-plaxo-surface rounded mb-2"></div>
                  <div className="h-4 bg-plaxo-surface rounded mb-6 w-1/2"></div>
                  <div className="h-12 bg-plaxo-surface rounded"></div>
                </div>
              ))
            ) : (
              ...
              Array.from({ length: 2 }).map((_, index) => (
                <div key={index} className="card animate-pulse">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-plaxo-surface rounded-xl"></div>
                    <div className="flex-1">
                      <div className="h-4 bg-plaxo-surface rounded mb-2"></div>
                      <div className="h-3 bg-plaxo-surface rounded w-1/2"></div>
                    </div>
                    <div className="w-20 h-8 bg-plaxo-surface rounded"></div>
                  </div>
                </div>
              ))
            ) : (
